...
export default function MathematicsPreview() {
  const { sessionId } = useParams();
  const navigate = useNavigate();
  const [previewSlots, setPreviewSlots] = useState<MathematicsSlot[]>([]);
  ...
  const loadPreview = async () => {
    try {
      setIsLoading(true);
      const res = await fetch(`/api/mathematics/preview/${sessionId}`);
  ...
  const handleRegenerate = async () => {
    ...
    try {
      const res = await fetch(`/api/mathematics/regenerate/${sessionId}`, { method: 'POST' });
      const data = await res.json();
  ...
  const handleFix = async () => {
    ...
    try {
      const res = await fetch(`/api/mathematics/fix/${sessionId}`, {
        method: 'POST',
      ...
      setStatus({ type: 'success', msg: 'Mathematics slots finalized successfully!' });
      setHasChanges(false);
      emitDataInvalidation(['staff_workload', 'mathematics', 'timetable', 'classes'], 'MathematicsPreview.handleFix');
  ...
  if (isLoading) {
    return <div className="text-cyan-400 font-mono p-8">Loading Mathematics preview...</div>;
  }

  const days = [1, 2, 3, 4, 5, 6];
  const classGroups = new Map<number, {
    preview: MathematicsSlot[];
    ...
    <div className="space-y-8 p-8">
      <header className="flex justify-between items-center border-b border-[#1e2d47] pb-6">
        <div className="flex items-center gap-4">
          ...
          <div>
            <div className="text-[10px] font-mono text-cyan-500 uppercase tracking-[0.2em]">Mathematics Timetable Preview</div>
          ...
          <section key={classId} className="bg-[#0f1623] border border-[#1e2d47] rounded-xl overflow-hidden shadow-2xl">
            <div className="bg-[#141c2e] px-6 py-4 border-b border-[#1e2d47] flex justify-between items-center gap-4">
              ...
              <div className="text-sm font-mono text-slate-400">
                Assigned: {classGroup.preview.length} Mathematics slots
              </div>
            </div>
                  ...
                  <tr>
                    ...
                    {periods.map(period => (
                      <Fragment key={`period-header-${classId}-${period}`}>
                        ...
                        {period === breakAfter && (
                          <th className="p-3 text-center text-[10px] font-mono text-orange-500 uppercase border-b border-[#1e2d47] bg-orange-500/5">Break</th>
                        )}
                        {period === lunchAfter && (
                          <th className="p-3 text-center text-[10px] font-mono text-cyan-500 uppercase border-b border-[#1e2d47] bg-cyan-500/5">Lunch</th>
                        )}
                      </Fragment>
                    ))}
                  </tr>
                  ...
                  {days.map(day => (
                    <tr key={`row-${classId}-${day}`} className="border-b border-[#1e2d47]/50 hover:bg-[#141c2e]/30 transition-colors">
                      ...
                      {periods.map(period => {
                        ...
                        const isMath = !!previewSlot;
                        const isOccupied = !!previewSlot || !!existingSlot;
                        const isBlockedByStaff = !!staffBusySlot && !isOccupied;
                        const slotTitle = isMath
                          ? previewSlot?.subject_name || 'Mathematics'
                          : existingSlot?.type === 'placement'
                            ? 'PLACEMENT'
                            : existingSlot?.subject_name || '-';
                        const slotSubtext = isMath
                          ? previewSlot?.subject_code || ''
                          : existingSlot?.type === 'placement'
                            ? 'TRAINING'
                            : existingSlot?.subject_code || '';
                        const slotMeta = isMath
                          ? previewSlot?.staff_name || ''
                          : existingSlot?.lab_name || existingSlot?.staff_name || '';

                        return (
                          <Fragment key={`cell-${classId}-${day}-${period}`}>
                            <td
                              className={clsx('p-2 transition-colors', isEditing && !isOccupied && !isBlockedByStaff ? 'hover:bg-cyan-500/10' : '')}
                            ...
                            >
                              {isOccupied ? (
                                <div
                                  ...
                                  onDragStart={(e) => previewSlot && handleDragStart(e, day, period, classId, previewSlot.subject_id)}
                                  className={clsx(
                                ...
                                >
                                  <div className={clsx('font-bold text-xs', isMath || existingSlot?.type === 'placement' ? 'text-emerald-400' : 'text-white')}>
                                    {slotTitle}
                                  </div>
                                  {slotSubtext && <div className="text-[10px] text-cyan-400 font-mono">{slotSubtext}</div>}
                                  {slotMeta && (
                                    <div className={clsx('text-[9px]', existingSlot?.lab_name ? 'text-emerald-400 font-mono' : 'text-slate-500')}>
                                      {slotMeta}
                                    </div>
                                  )}
                                </div>
            ...
            <div className="mt-4 text-[10px] font-mono text-slate-500">
              Total hours/week: {classGroup.preview.length} | Subjects: {new Set(classGroup.preview.map(s => s.subject_id)).size}
            </div>
          </section>
        ))}
      </div>
      ...
      {previewSlots.length === 0 && (
        <div className="rounded-lg border border-amber-500/30 bg-amber-500/10 px-4 py-4 text-center">
          <p className="text-sm text-amber-300 font-mono">No Mathematics slots scheduled for this session.</p>
        </div>
      )}

      {previewSlots.length > 0 && filteredClassGroups.length === 0 && (
        <div className="rounded-lg border border-amber-500/30 bg-amber-500/10 px-4 py-4 text-center">
          <p className="text-sm text-amber-300 font-mono">No classes match the selected filters.</p>
        </div>
      )}
    </div>
  );
