...
export default function EnglishPreview() {
  ...
  const [previewSlots, setPreviewSlots] = useState<EnglishSlot[]>([]);
  ...
  const handleRegenerate = async () => {
    ...
    try {
      const res = await fetch(`/api/english/regenerate/${sessionId}`, { method: 'POST' });
      const data = await res.json();
  ...
  const handleFix = async () => {
    ...
    try {
      const res = await fetch(`/api/english/fix/${sessionId}`, {
        method: 'POST',
      ...
      setStatus({ type: 'success', msg: 'English slots finalized successfully!' });
      setHasChanges(false);
      emitDataInvalidation(['staff_workload', 'english', 'timetable', 'classes'], 'EnglishPreview.handleFix');
  ...
  if (isLoading) {
    return <div className="text-cyan-400 font-mono p-8">Loading English preview...</div>;
  }
    ...
    <div className="space-y-8 p-8">
      <header className="flex justify-between items-center border-b border-[#1e2d47] pb-6">
        <div className="flex items-center gap-4">
          ...
          <div>
            <div className="text-[10px] font-mono text-cyan-500 uppercase tracking-[0.2em]">English Timetable Preview</div>
          ...
          <section key={classId} className="bg-[#0f1623] border border-[#1e2d47] rounded-xl overflow-hidden shadow-2xl">
            <div className="bg-[#141c2e] px-6 py-4 border-b border-[#1e2d47] flex justify-between items-center gap-4">
              ...
              <div className="text-sm font-mono text-slate-400">
                Assigned: {classGroup.preview.length} English slots
              </div>
            </div>
                            ...
                            >
                              {isOccupied ? (
                                ...
                                >
                                  <div className={clsx(
                                    'font-bold text-xs',
                                    isEnglish || existingSlot?.type === 'placement' ? 'text-emerald-400' : 'text-white'
                                  )}>
                                    {slotTitle}
                                  </div>
                                  {slotSubtext && (
      ...
      {previewSlots.length === 0 && (
        <div className="rounded-lg border border-amber-500/30 bg-amber-500/10 px-4 py-4 text-center">
          <p className="text-sm text-amber-300 font-mono">No English slots scheduled for this session.</p>
        </div>
      )}
